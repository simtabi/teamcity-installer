...
#
# Setting a password afterwards lives in users.sh, including for this account.
# Two implementations of "give this user a password" is how the rest of this
# console ended up with the same defect in three places.
...
admin::bootstrap() {
    ...
    if [[ -z ${TC_ADMIN_PASSWORD:-} ]]; then
        ...
        # Said here, where the risk is created, rather than in a document read
        # only after someone is already locked out.
        ui::note "If you lose it:  ./tc users passwd $user  — no password needed."
    fi
